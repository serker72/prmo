...
if($_GET['term']) 	{
	...
	"" as login
	 ...
	 where ((p.is_org=0 and p.org_id="'.$result['org_id'].'") or (p.is_org=1 and p.id<>"'.$result['org_id'].'")) and p.is_active=1 and p.full_name like "%'.iconv("utf-8","windows-1251",SecStr($_GET['term'])).'%" )
	UNION distinct
	(select p.id, p.name_s as full_name,
	"" as opf_name,
	"" as code,
	p.login as login
	
	 from user as p where p.is_active=1 and p.name_s like "%'.iconv("utf-8","windows-1251",SecStr($_GET['term'])).'%" )
	 order by 1
	';
	//echo $sql;
	...
	for($i=0; $i<$rc; $i++){
		...
		$vv=array();
		if(strlen($v['code'])==0) $vv['id']='U'.$v['id'];
		else $vv['id']=$v['id'];
		
		if(strlen($v['code'])==0) $vv['text']=iconv('windows-1251','utf-8',$v['login'].' '.$v['full_name']);
		else $vv['text']=iconv('windows-1251','utf-8',$v['code'].' '.$v['full_name'].', '.$v['opf_name']);
