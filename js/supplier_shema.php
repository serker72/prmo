...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',318)){
		...
		$_ff=new FileDocFolderItem(3,abs((int)$_POST['sup_id']), new Supplier_Sh_Item(3));
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['sup_id']), new Supplier_Sh_Item(3), new FileDocFolderItem(3,abs((int)$_POST['sup_id']), new Supplier_Sh_Item(3)));   //FileFolderList($storage_id);
	$_ff->setTableName('supplier_shema_file_folder');
