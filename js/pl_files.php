...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,602,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',602)){
		...
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new PlPosFileItem(4));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,602,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',69)){
		...
		$_ff->Edit($id, $params,NULL,$result,69);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,602,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['bill_id']), new PlPosFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new PlPosFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('pl_position_file_folder');
