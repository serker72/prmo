...
if(!$au->user_rights->CheckAccess('w',889)&&!$au->user_rights->CheckAccess('w',890)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,889,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',888)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(1,abs((int)$_POST['pay_id']), new CashInFileItem(1));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,888,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
		
	}
}
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',889)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(1,abs((int)$_POST['sup_id']), new CashInFileItem(1));
		$_ff->SetTablename('cash_in_file_folder');
		$_ff->SetDocIdName('pay_id');
		...
		$_ff->Edit($id, $params,NULL,$result,889);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,889,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['pay_id']), new CashInFileItem(1), new FileDocFolderItem(1,abs((int)$_POST['pay_id']), new CashInFileItem(1)));   //FileFolderList($storage_id);
	$_ff->setTableName('cash_in_file_folder');
