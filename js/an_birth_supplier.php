...
if($_GET['term']) 	{
	...
	if($au->FltSupplier($result)){  
		...
		$supplier_flt=' and p.id in('.implode(', ', $limited_supplier).') ';
		
		//$dec->AddEntry(new SqlEntry('p.id', NULL, SqlEntry::IN_VALUES, NULL,$limited_supplier));	
	}
	
	//ограничения по ответственным
	if(! $au->user_rights->CheckAccess('w',909)){
		
		$supplier_flt.=' and p.id in(select distinct supplier_id from supplier_responsible_user where user_id="'.$result['id'].'") ';
	}
