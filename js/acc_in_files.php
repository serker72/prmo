...
if(!$au->user_rights->CheckAccess('w',669)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,669,NULL,$txt);
	...
	if(!$au->user_rights->CheckAccess('w',669)&&!$au->user_rights->CheckAccess('w',825)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',668)){
		...
		$_ff=new FileDocFolderItem(3,abs((int)$_POST['acc_id']), new AccInFileItem(3));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,668,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',669)){
		...
		$_ff->Edit($id, $params,NULL,$result,669);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,669,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['acc_id']), new AccInFileItem(3), new FileDocFolderItem(3,abs((int)$_POST['acc_id']), new AccInFileItem(3)));   //FileFolderList($storage_id);
	$_ff->setTableName('acceptance_file_folder');
