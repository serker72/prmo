...
if(!$au->user_rights->CheckAccess('w',299)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,299,NULL,$txt);
	...
	if(!$au->user_rights->CheckAccess('w',299)&&!$au->user_rights->CheckAccess('w',605)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',298)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['komplekt_ved_id']), new KvFileItem(4));
		
		$params=array();
		$params['doc_id']=abs((int)$_POST['komplekt_ved_id']);
		...
		$log->PutEntry($result['id'],'создал папку',NULL,298,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',299)){
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,299,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_fg=new FileDocFolderList($storage_id, abs((int)$_POST['komplekt_ved_id']), new KvFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['komplekt_ved_id']), new KvFileItem(4)));   //FileFolderList($storage_id);
	
	
	$fld=$_fg->GetItemsArr($selected_folders);
