...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал договору описание',NULL,87,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',171)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(3,abs((int)$_POST['sup_id']), new ContractItem(3));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,171,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
		
	}
}
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',172)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
		...
		$_ff->Edit($id, $params,NULL,$result,172);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,172,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['sup_id']), new ContractItem(3), new FileDocFolderItem(3,abs((int)$_POST['sup_id']), new ContractItem(3)));   //FileFolderList($storage_id);
	$_ff->setTableName('contract_file_folder');
