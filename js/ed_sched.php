...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу заметки описание',NULL,905,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	 if(!$au->user_rights->CheckAccess('w',905)){
		...
		$_ff=new FileDocFolderItem(1,abs((int)$_POST['id']), new SchedFileItem(1));
		$_ff->SetTablename('sched_file_folder');
		$_ff->SetDocIdName('id');
		
		$params=array();
		$params['doc_id']=abs((int)$_POST['id']);
	...
	//	$params['org_id']=$result['org_id'];
		...
		$log->PutEntry($result['id'],'создал папку',NULL,905,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['id']), new SchedFileItem(1), new FileDocFolderItem(1,abs((int)$_POST['id']), new SchedFileItem(1)));   //FileFolderList($storage_id);
	$_ff->setTableName('sched_file_folder');
