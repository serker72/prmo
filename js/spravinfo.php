...
/*if(!$au->user_rights->CheckAccess('w',31)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,31,NULL,$txt,$id);
	...
	/*if(!$au->user_rights->CheckAccess('w',61)&&!$au->user_rights->CheckAccess('w',823)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	...
	/*if(!$au->user_rights->CheckAccess('w',564)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
		...
		$log->PutEntry($result['id'],'создал папку',NULL,564,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
		
	//}
}
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	/*if(!$au->user_rights->CheckAccess('w',565)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{*/
		$_ff=new FileFolderItem;
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,565,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$fld=$_fg->GetItemsArr($selected_folders,35,$result);
