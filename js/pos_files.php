...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	if(!$au->user_rights->CheckAccess('w',68)){
		...
		$_fi->Edit($id, array('txt'=>$txt));
		$log->PutEntry($result['id'],'дал файлу описание',NULL,68,NULL,$txt);
	}
}
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',68)){
		...
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new PosFileItem(4));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,68,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',69)){
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,69,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['bill_id']), new PosFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new PosFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('catalog_position_file_folder');
