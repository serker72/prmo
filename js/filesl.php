...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,557,NULL,$txt,$id);
	...
	/*if(!$au->user_rights->CheckAccess('w',576)){
		...
		$log->PutEntry($result['id'],'создал папку',NULL,576,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	/*if(!$au->user_rights->CheckAccess('w',577)){
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,577,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
