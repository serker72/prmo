...
$_fi=new UserVPasportItem(2); //ContractItem;
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал описание документу сотрудника',NULL,775,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',775)){
		...
		$_ff=new FileDocFolderItem(2,abs((int)$_POST['user_id']), new UserVPasportItem(2));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,775,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
		
	}
}
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',776)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
		...
		$_ff->Edit($id, $params,NULL,$result,776);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,776,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['user_id']), new UserVPasportItem(2), new FileDocFolderItem(2,abs((int)$_POST['user_id']), new UserVPasportItem(2)));   //FileFolderList($storage_id);
	$_ff->setTableName('user_pasport_file_folder');
