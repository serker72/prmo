...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,87,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',829)){
		...
		$_ff=new FileDocFolderItem(6,abs((int)$_POST['sup_id']), new  SupplierFileItem(6));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,829,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',830)){
		...
		$_ff->Edit($id, $params,NULL,$result,830);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,830,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['sup_id']), new SupplierFileItem(6), new FileDocFolderItem(6,abs((int)$_POST['sup_id']), new SupplierFileItem(6)));   //FileFolderList($storage_id);
	$_ff->setTableName('supplier_shema_file_folder');
