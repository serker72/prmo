...
/*if(!$au->user_rights->CheckAccess('w',561)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,561,NULL,$txt,$id);
	...
	/*if(!$au->user_rights->CheckAccess('w',562)&&!$au->user_rights->CheckAccess('w',827)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	/*if(!$au->user_rights->CheckAccess('w',580)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
		...
		$log->PutEntry($result['id'],'создал папку',NULL,580,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
		
	//}
}
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	/*if(!$au->user_rights->CheckAccess('w',581)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{*/
		$_ff=new FileFolderItem(5);
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,581,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$fld=$_fg->GetItemsArr($selected_folders,47,$result);
