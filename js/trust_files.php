...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,208,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',208)){
		...
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['trust_id']), new TrustFileItem(4));
		$_ff->SetTablename('trust_file_folder');
		$_ff->SetDocIdName('trust_id');
		
		$params=array();
		$params['doc_id']=abs((int)$_POST['trust_id']);
		...
		$log->PutEntry($result['id'],'создал папку',NULL,208,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',208)){
		...
		$_ff->Edit($id, $params,NULL,$result,208);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,208,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['trust_id']), new TrustFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['trust_id']), new TrustFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('trust_file_folder');
