...
if(!$au->user_rights->CheckAccess('w',329)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,329,NULL,$txt);
	...
	if(!$au->user_rights->CheckAccess('w',329)&&!$au->user_rights->CheckAccess('w',818)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',328)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new InvFileItem(4));
		$_ff->SetTablename('inventory_file_folder');
		$_ff->SetDocIdName('bill_id');
		
		$params=array();
		$params['doc_id']=abs((int)$_POST['bill_id']);
		...
		$log->PutEntry($result['id'],'создал папку',NULL,328,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',329)){
		...
		$_ff->Edit($id, $params,NULL,$result,329);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,329,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['bill_id']), new InvFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new InvFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('inventory_file_folder');
