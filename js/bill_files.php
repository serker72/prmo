...
if(!$au->user_rights->CheckAccess('w',193)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,193,NULL,$txt);
	...
	if(!$au->user_rights->CheckAccess('w',193)&&!$au->user_rights->CheckAccess('w',821)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',192)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new BillFileItem(4));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,192,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',193)){
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,193,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['bill_id']), new BillFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new BillFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('bill_file_folder');
