...
if($_GET['term']) 	{
	...
	$sql='select p.*, opf.name as opf_name from supplier as p left join opf on opf.id=p.opf_id where ((p.is_org=0 and p.org_id="'.$result['org_id'].'") or (p.is_org=1 and p.id<>"'.$result['org_id'].'")) and p.is_active=1 and p.full_name like "%'.iconv("utf-8","windows-1251",SecStr($_GET['term'])).'%" ';
	//echo $sql;
