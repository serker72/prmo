...
require_once('../classes/pay_in_view.class.php');
...
if(isset($_POST['action'])&&($_POST['action']=="redraw_notes")){
	...
	$sm->assign('can_edit', $au->user_rights->CheckAccess('w',684));
	
	
	$ret=$sm->fetch('pay_in/d_notes.html');
	
	
}elseif(isset($_POST['action'])&&($_POST['action']=="add_notes")){
	//dostup
	if(!$au->user_rights->CheckAccess('w',684)){
		header("HTTP/1.1 403 Forbidden");
		  header("Status: 403 Forbidden");	
		  die();	
	}
	...
	$log->PutEntry($result['id'],'добавил примечания к входящей оплате', NULL,684, NULL,SecStr(iconv("utf-8","windows-1251",$_POST['note']),9),$user_id);
	...
	$log->PutEntry($result['id'],'редактировал примечания по входящей оплате', NULL,684,NULL, SecStr(iconv("utf-8","windows-1251",$_POST['note']),9),$user_id);
	...
	$log->PutEntry($result['id'],'удалил примечания по входящей оплате', NULL,684,NULL,NULL,$user_id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_confirm")){
	...
	/*if($shorter==0) $template='pay_in/all_pays_list.html';
	else $template='pay_in/pays_list.html';
	*/
	if($shorter==0){
		 $template='pay_in/all_pays_list.html';
		  $prefix='_in';
	}else{
		   $prefix='_pay';
		 $template='pay_in/pays_list.html';
	}
	...
	if($shorter==0) $ret=$acg->ShowAllPos($template, $dec, $au->user_rights->CheckAccess('w',683)||$au->user_rights->CheckAccess('w',693), $au->user_rights->CheckAccess('w',691) ,0,100, $au->user_rights->CheckAccess('w',689),  $au->user_rights->CheckAccess('w',96),false,true,$au->user_rights->CheckAccess('w',692),$au->user_rights->CheckAccess('w',690), $au->user_rights->CheckAccess('w',693));
	else $ret=$acg->ShowAllPos($template, $dec, $au->user_rights->CheckAccess('w',683)||$au->user_rights->CheckAccess('w',693), $au->user_rights->CheckAccess('w',691),0,100,$au->user_rights->CheckAccess('w',689),  $au->user_rights->CheckAccess('w',96),false,true, $au->user_rights->CheckAccess('w',692),$au->user_rights->CheckAccess('w',690), $au->user_rights->CheckAccess('w',693));
...
/*	if($shorter==0) $template='pay_in/all_pays_list.html';
	...
	if(($trust['status_id']==14)&&($trust['is_confirmed']==0)){
		//удаление	
		if($au->user_rights->CheckAccess('w',691)){
			$_ti->Edit($id,array('status_id'=>3, 'confirm_pdate'=>time(), 'user_confirm_id'=>$result['id']));	
			
			$stat=$_stat->GetItemById(3);
			$log->PutEntry($result['id'],'аннулирование входящей оплаты',NULL,93,NULL,'входящая оплата № '.$trust['code'].': установлен статус '.$stat['name'],$trust['bill_id']);	
			
			$log->PutEntry($result['id'],'аннулирование входящей оплаты',NULL,691,NULL,'входящая оплата № '.$trust['code'].': установлен статус '.$stat['name'],$trust['id']);
		...
		//разудаление
		if($au->user_rights->CheckAccess('w',692)){
			$_ti->Edit($id,array('status_id'=>14, 'confirm_pdate'=>time(), 'user_confirm_id'=>$result['id']));
			
			$stat=$_stat->GetItemById(14);
			$log->PutEntry($result['id'],'восстановление входящей оплаты',NULL,93,NULL,'входящая оплата № '.$trust['code'].': установлен статус '.$stat['name'],$trust['bill_id']);		
			
			$log->PutEntry($result['id'],'восстановление входящей оплаты',NULL,692,NULL,'входящая оплата № '.$trust['code'].': установлен статус '.$stat['name'],$trust['id']);	
	...
	if($from_card==0){
	  ...
	  if($shorter==0) $ret=$acg->ShowAllPos($template, $dec, $au->user_rights->CheckAccess('w',683)||$au->user_rights->CheckAccess('w',693), $au->user_rights->CheckAccess('w',691),0,100,$au->user_rights->CheckAccess('w',689),  $au->user_rights->CheckAccess('w',96),false,true,$au->user_rights->CheckAccess('w',692), $au->user_rights->CheckAccess('w',690));
	  else {
		  //$_bi=new BillItem;
		 ...
		 // echo 'zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz';
		  ...
		  $ret=$acg->ShowPos($bill_id,  $trust['supplier_id'], $template, $dec2, $au->user_rights->CheckAccess('w',683)||$au->user_rights->CheckAccess('w',693), $au->user_rights->CheckAccess('w',691),$au->user_rights->CheckAccess('w',689),  $au->user_rights->CheckAccess('w',96), false,true,$au->user_rights->CheckAccess('w',692),$au->user_rights->CheckAccess('w',690));
		...
		$editing_user['can_annul']=$_ti->DocCanAnnul($editing_user['id'],$reason)&&$au->user_rights->CheckAccess('w',691);
		if(!$au->user_rights->CheckAccess('w',691)) $reason='недостаточно прав для данной операции';
		$editing_user['can_annul_reason']=$reason;
		
		
		$editing_user['can_restore']=$_ti->DocCanRestore($editing_user['id'],$reason)&&$au->user_rights->CheckAccess('w',692);
			if(!$au->user_rights->CheckAccess('w',692)) $reason='недостаточно прав для данной операции';
		
		
		//$editing_user['binded_to_annul']=$_ti->GetBindedDocumentsToAnnul($editing_user['id']);
		
		$sm->assign('ship',$editing_user);
		$ret=$sm->fetch('pay_in/toggle_annul_card.html');	
	}
...
//работа с поставщиком	
	...
	$ret=$_pg->GetItemsForPay('pay_in/suppliers_list.html',  $dec,true,$all7,$result);
	...
	$ret=$sm->fetch('pay_in/bdetails_list.html');
	...
	$ret=$sm->fetch('pay_in/contracts_list.html');
	...
	if($pay_mode==1){ 
		$ret.=$sm->fetch("pay_in/positions_editbybill_set.html");
	}else $ret.=$sm->fetch("pay_in/positions_edit_set.html");
	...
	$ret=$sm->fetch("pay_in/positions_on_page_set.html");
	...
	$ret.=$sm->fetch("pay_in/positions_edit_rows_filter.html");
	...
	$ret=$_kr->InPays($id,'pay_in/in_pays.html',$result['org_id'],true,$except_id, $kind);
...
elseif(isset($_POST['action'])&&(($_POST['action']=="mode_reestr"))){
	$_views=new Pay_In_ViewGroup;
	$_view=new Pay_In_ViewItem;
