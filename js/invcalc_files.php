...
if(!$au->user_rights->CheckAccess('w',456)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,456,NULL,$txt);
	...
	if(!$au->user_rights->CheckAccess('w',456)&&!$au->user_rights->CheckAccess('w',819)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',455)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new InvCalcFileItem(4));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,455,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',456)){
		...
		$_ff->Edit($id, $params,NULL,$result,456);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,456,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['bill_id']), new InvCalcFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['bill_id']), new InvCalcFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('invcalc_file_folder');
