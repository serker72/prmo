...
/*if(!$au->user_rights->CheckAccess('w',30)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,30,NULL,$txt,$id);
	...
	/*if(!$au->user_rights->CheckAccess('w',60)&&!$au->user_rights->CheckAccess('w',824)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	...
	/*if(!$au->user_rights->CheckAccess('w',572)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
		...
		$log->PutEntry($result['id'],'создал папку',NULL,572,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
		
	//}
}
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	/*if(!$au->user_rights->CheckAccess('w',573)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,573,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$fld=$_fg->GetItemsArr($selected_folders,37,$result);
