...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу планировщика описание',NULL,905,NULL,$txt);
	...
	/*if(!$au->user_rights->CheckAccess('w',809)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	 if(!$au->user_rights->CheckAccess('w',905)){
		...
		$_ff=new FileDocFolderItem(1,abs((int)$_POST['id']), new SchedOtherFileItem(1));
	...
	//	$params['org_id']=$result['org_id'];
		...
		$log->PutEntry($result['id'],'создал папку',NULL,905,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$params['doc_id']);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	 if(!$au->user_rights->CheckAccess('w',905)){
		...
		$_ff->Edit($id, $params,NULL,$result, 93);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,905,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['bill_id']), new SchedOtherFileItem(1), new FileDocFolderItem(1,abs((int)$_POST['bill_id']), new SchedOtherFileItem(1)));   //FileFolderList($storage_id);
	$_ff->setTableName('sched_file_folder');
