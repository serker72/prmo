...
if(!$au->user_rights->CheckAccess('w',687)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,687,NULL,$txt);
	...
	if(!$au->user_rights->CheckAccess('w',687)&&!$au->user_rights->CheckAccess('w',816)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',686)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(3,abs((int)$_POST['pay_id']), new PayInFileItem(3));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,686,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',687)){
		...
		$_ff=new FileDocFolderItem(3,abs((int)$_POST['sup_id']), new PayInFileItem(3));
		$_ff->SetTablename('payment_file_folder');
		$_ff->SetDocIdName('pay_id');
		...
		$_ff->Edit($id, $params,NULL,$result,687);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,687,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['pay_id']), new PayInFileItem(3), new FileDocFolderItem(3,abs((int)$_POST['pay_id']), new PayInFileItem(3)));   //FileFolderList($storage_id);
	$_ff->setTableName('payment_file_folder');
