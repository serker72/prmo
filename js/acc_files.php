...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,238,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',237)){
		...
		$log->PutEntry($result['id'],'создал папку',NULL,237,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',238)){
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,238,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
