...
/*if(!$au->user_rights->CheckAccess('w',477)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,477,NULL,$txt,$id);
	...
	/*
	if(!$au->user_rights->CheckAccess('w',478)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
//	}
	...
	/*if(!$au->user_rights->CheckAccess('w',568)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
		...
		$log->PutEntry($result['id'],'создал папку',NULL,568,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
		
	//}
}
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	/*if(!$au->user_rights->CheckAccess('w',569)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,569,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$fld=$_fg->GetItemsArr($selected_folders,36,$result);
