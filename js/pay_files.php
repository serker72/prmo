...
if(!$au->user_rights->CheckAccess('w',275)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,275,NULL,$txt);
	...
	if(!$au->user_rights->CheckAccess('w',275)&&!$au->user_rights->CheckAccess('w',820)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',274)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['pay_id']), new PayFileItem(4));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,274,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',275)){
		...
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['sup_id']), new PayFileItem(4));
		$_ff->SetTablename('payment_file_folder');
		$_ff->SetDocIdName('pay_id');
		...
		$_ff->Edit($id, $params,NULL,$result,275);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,275,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['pay_id']), new PayFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['pay_id']), new PayFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('payment_file_folder');
