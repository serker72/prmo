...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,706,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',705)){
		...
		$log->PutEntry($result['id'],'создал папку',NULL,705,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',706)){
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,706,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
