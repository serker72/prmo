...
$_fi=new UserPasportItem(1); //ContractItem;
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал описание документу сотрудника',NULL,119,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',119)){
		...
		$_ff=new FileDocFolderItem(1,abs((int)$_POST['user_id']), new UserPasportItem(1));
		$_ff->SetTablename('user_pasport_file_folder');
		$_ff->SetDocIdName('user_id');
		
		$params=array();
		$params['doc_id']=abs((int)$_POST['user_id']);
		...
		$log->PutEntry($result['id'],'создал папку',NULL,119,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
		
	}
}
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',158)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['user_id']), new UserPasportItem(4));
		$_ff->SetTablename('user_pasport_file_folder');
		$_ff->SetDocIdName('user_id');
		...
		$_ff->Edit($id, $params,NULL,$result,158);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,158,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['user_id']), new UserPasportItem(1), new FileDocFolderItem(1,abs((int)$_POST['user_id']), new UserPasportItem(1)));   //FileFolderList($storage_id);
	$_ff->setTableName('user_pasport_file_folder');
