...
$user=$_user->GetItemById($fmi['sched_id']);

if($fmi['folder_id']==0){
	$_folder_desrc='';
}else{
	$_ff=new FileDocFolderItem(1, $fmi['prikaz_id'], new SchedFileItem(1));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл планировщика',NULL, 903, NULL,SecStr('файл '.$fmi['orig_name'].' для заметки '.  $user['id'].' '.$_folder_desrc),$user['id']);
