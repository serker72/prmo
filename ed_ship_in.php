...
require_once('classes/sh_i_in_posgroup.php');
require_once('classes/sh_i_in_item.php');
...
$smarty->assign("SITETITLE",'Редактирование распоряжения на приемку');
...
if($action==0){
	...
	$editing_user=$_ship->GetItemByFields(array('id'=>$id,'is_incoming'=>1));
...
if($print!=0){
	if(!$au->user_rights->CheckAccess('w',653)){
	  header("HTTP/1.1 403 Forbidden");
	  header("Status: 403 Forbidden");
	  include("403.php");
	  die();	
...
if(($action==0)&&(isset($_POST['doNew'])||isset($_POST['doNewEdit']))){
	if(!$au->user_rights->CheckAccess('w',640)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	...
	//запись в журнале
	if($code>0){
		$log->PutEntry($result['id'],'создал распоряжение на приемку по входящему счету',NULL,613,NULL,NULL,$bill_id);	
		
		
		$log->PutEntry($result['id'],'создал распоряжение на приемку по входящему счету',NULL,640,NULL,NULL,$code);	
		
		
	}
	
	
	if(($code>0)&&($au->user_rights->CheckAccess('w',641))){
		//позиции
		$positions=array();
		...
		foreach($_POST as $k=>$v){
			if(eregi("^new_hash_([0-9a-z]+)",$k)){
			  ...
			  $positions[]=array(
				  ...
				  'out_bill_id'=>abs((int)$_POST['new_out_bill_id_'.$hash]),
		...
		foreach($positions as $k=>$v){
			...
			if($pos!==false) {
				...
				$log->PutEntry($result['id'],'добавил позицию распоряжения на приемку', NULL, 613,NULL,$descr,$bill_id);	
				
				$log->PutEntry($result['id'],'добавил позицию распоряжения на приемку', NULL, 641,NULL,$descr,$code);	
	...
	if(isset($_POST['doNew'])){
		...
		if(!$au->user_rights->CheckAccess('w',644)){
			header("HTTP/1.1 403 Forbidden");
			header("Status: 403 Forbidden");
			include("403.php");
			die();	
		}
		header("Location: ed_ship_in.php?action=1&id=".$code.'&from_begin=1');
		die();	
		...
		foreach($params as $k=>$v){
			...
			if(addslashes($editing_user[$k])!=$v){
				...
				if($k=='pdate_shipping_plan'){
					$log->PutEntry($result['id'],'редактировал плановую дату поставки',NULL,613,NULL,'дата: '.$_POST['pdate_shipping_plan'],$bill_id);
					
					$log->PutEntry($result['id'],'редактировал плановую дату поставки',NULL,644,NULL,'дата: '.$_POST['pdate_shipping_plan'],$id);
					continue;	
				}
				
				$log->PutEntry($result['id'],'редактировал распоряжение на приемку',NULL,613, NULL, 'в поле '.$k.' установлено значение '.$v,$bill_id);
				
				$log->PutEntry($result['id'],'редактировал распоряжение на приемку',NULL,644, NULL, 'в поле '.$k.' установлено значение '.$v,$id);
			}
			
		}
		
		if($au->user_rights->CheckAccess('w',641)){
		  $positions=array();
		  ...
		  foreach($log_entries as $k=>$v){
			  ...
			  if($v['action']==0){
				  $log->PutEntry($result['id'],'добавил позицию распоряжения на приемку',NULL,613,NULL,$description,$bill_id);	
				  $log->PutEntry($result['id'],'добавил позицию распоряжения на приемку',NULL,641,NULL,$description,$id);	
			  }elseif($v['action']==1){
				  $log->PutEntry($result['id'],'редактировал позицию распоряжения на приемку',NULL,613,NULL,$description,$bill_id);
				  
				  $log->PutEntry($result['id'],'редактировал позицию распоряжения на приемку',NULL,642,NULL,$description,$id);
				  
			  }elseif($v['action']==2){
				  $log->PutEntry($result['id'],'удалил позицию распоряжения на приемку',NULL,613,NULL,$description,$bill_id);
				  
				  $log->PutEntry($result['id'],'удалил позицию распоряжения на приемку',NULL,643,NULL,$description,$id);
	...
	if($editing_user['is_confirmed']==1){
		...
		if(($au->user_rights->CheckAccess('w',652))||$au->user_rights->CheckAccess('w',96)){
			if(!isset($_POST['is_confirmed'])&&in_array($editing_user['status_id'],array(2,7,8))&&in_array($_POST['current_status_id'],array(2,7,8))){
				...
				if($_ship->DocCanUnconfirm($id,$reas)){
				  ...
				  $log->PutEntry($result['id'],'снял утверждение распоряжения на приемку',NULL,613, NULL, NULL,$bill_id);
				  
				  $log->PutEntry($result['id'],'снял утверждение распоряжения на приемку',NULL,652, NULL, NULL,$id);	
				}
			}
		}else{
			//нет прав	
		...
		//есть права
		if($au->user_rights->CheckAccess('w',651)||$au->user_rights->CheckAccess('w',96)){
			if(isset($_POST['is_confirmed'])&&($editing_user['status_id']==1)&&($_POST['current_status_id']==1)){
				...
				$log->PutEntry($result['id'],'утвердил распоряжение на приемку',NULL,613, NULL, NULL,$bill_id);	
				
				$log->PutEntry($result['id'],'утвердил распоряжение на приемку',NULL,651, NULL, NULL,$id);	
	...
	if(isset($_POST['doEdit'])){
		...
		header("Location: ed_ship_in.php?action=1&id=".$id.'&from_begin='.$from_begin);
		die();	
...
unset($smarty);
		...
		foreach($input_array as $kk=>$vv){
			...
			$sql='select p.id as p_id, p.bill_id, p.position_id as position_id, p.pl_position_id as pl_position_id,
				p.pl_discount_id, p.pl_discount_value, p.pl_discount_rub_or_percent,	
				 ...
				 p.out_bill_id, b.code as out_bill_code			 
	...
	where 
		...
		and p.out_bill_id="'.$vv['out_bill_id'].'" 
		...
		//echo $sql.'<br>';
			...
			$h['not_in_bill']=$h['quantity']- $_mf->MaxInShI($h['bill_id'], $h['position_id'], $h['pl_position_id'],  $h['pl_discount_id'],  $h['pl_discount_value'],  $h['pl_discount_rub_or_percent'],  NULL, $h['out_bill_id']);
			
			$h['in_acc']=$_mf->MaxInAcc($h['bill_id'],  $h['position_id'], $h['pl_position_id'],  $h['pl_discount_id'],  $h['pl_discount_value'],  $h['pl_discount_rub_or_percent'],0, 0,$h['out_bill_id']);
			...
			if($vv['quantity']>0){
			  ...
			  $positions[]=array(
				 // 'hash'=>md5($pos_id.'_'.$h['komplekt_ved_id']),
				  'hash'=>md5($h['pl_position_id'].'_'.$h['position_id'].'_'.$h['pl_discount_id'].'_'.$h['pl_discount_value'].'_'.$h['pl_discount_rub_or_percent'].'_'.$h['out_bill_id']),
		...
		$sm1->assign('can_edit_quantities',$au->user_rights->CheckAccess('w',657)); 
		
		$sm1->assign('can_add_positions',$au->user_rights->CheckAccess('w',641)); 
		$sm1->assign('can_del_positions',$au->user_rights->CheckAccess('w',643)); 
		
		$sm1->assign('can_eq',$au->user_rights->CheckAccess('w',656)); 
		...
		$sm1->assign('can_create',$au->user_rights->CheckAccess('w',640)); 
		$sm1->assign('can_edit',$au->user_rights->CheckAccess('w',644)); 
		
		$user_form=$sm1->fetch('ships_in/ship_create.html');
		
		$sm->assign('accs','В данном режиме просмотр поступлений по распоряжению на приемку недоступен.<br />
 Пожалуйста, нажмите кнопку "Создать распоряжение и перейти к утверждению" на вкладке "Распоряжение на приемку" для получения возможности просмотра поступлений.');	
		...
		$ships=$_acg->ShowPos($editing_user['bill_id'],'bills_in/fact_dates.html', $dec2,  $au->user_rights->CheckAccess('w',664), $au->user_rights->CheckAccess('w',664),  $au->user_rights->CheckAccess('w',671),  $au->user_rights->CheckAccess('w',96));
		...
		$editing_user['can_annul']=$_ship->DocCanAnnul($editing_user['id'],$reason)&&$au->user_rights->CheckAccess('w',654);
		if(!$au->user_rights->CheckAccess('w',654)) $reason='недостаточно прав для данной операции';
		$editing_user['can_annul_reason']=$reason;
		
		$editing_user['binded_to_annul']=$_ship->GetBindedDocumentsToAnnul($editing_user['id']);
		
		$editing_user['can_restore']=$_ship->DocCanRestore($editing_user['id'],$reason)&&$au->user_rights->CheckAccess('w',655);
			if(!$au->user_rights->CheckAccess('w',655)) $reason='недостаточно прав для данной операции';
		...
		if($editing_user['is_confirmed']==1){
			...
			}elseif($au->user_rights->CheckAccess('w',652)){
			...
			//95
			$can_confirm_price=$au->user_rights->CheckAccess('w',651)&&($editing_user['status_id']==1);
		}
		...
		$cannot_edit_reason='';
		$sm1->assign('can_edit_quantities',$au->user_rights->CheckAccess('w',657)&&(in_array($editing_user['status_id'],$_editable_status_id))&&$_ship->CanEditQuantities($editing_user['id'],$cannot_edit_reason,$editing_user)); 
		...
		$sm1->assign('can_print',$au->user_rights->CheckAccess('w',653)); 
		
		$sm1->assign('can_create',$au->user_rights->CheckAccess('w',640)); 
		$sm1->assign('can_edit',$au->user_rights->CheckAccess('w',644)); 
		
		$sm1->assign('can_make_acceptance', $au->user_rights->CheckAccess('w',661));
		...
		$cannot_select_positions=$cannot_select_positions&&!$au->user_rights->CheckAccess('w',661);
		$cannot_select_positions=$cannot_select_positions&&!$au->user_rights->CheckAccess('w',656);
		
		$cannot_select_positions=$cannot_select_positions&&(!$au->user_rights->CheckAccess('w',657));
		...
		$user_form=$sm1->fetch('ships_in/ship_edit'.$print_add1.'.html');
		...
		$_acg->SetPagename('ed_ship_in.php');
		...
		$_acg->SetAuthResult($result);
		$accs=$_acg->ShowPos($editing_user['bill_id'],'acc_in/acc_list.html', $dec2,
		 $au->user_rights->CheckAccess('w',664)||$au->user_rights->CheckAccess('w',673),
		 $au->user_rights->CheckAccess('w',674), 
		 $au->user_rights->CheckAccess('w',671), 
		 $au->user_rights->CheckAccess('w',96),true,false,
		 $au->user_rights->CheckAccess('w',675), $limited_sector, NULL,
		 $au->user_rights->CheckAccess('w',672) );
		...
		if($au->user_rights->CheckAccess('w',658)){
			...
			$decorator->AddEntry(new SqlEntry('object_id',NULL, SqlEntry::IN_VALUES, NULL, array(638,
639,
640,
641,
642,
...
658
			...
			$llg=$log->ShowLog('syslog/log.html',$decorator,$from,$to_page,'ed_ship_in.php',true,true,true);
	...
	$content=$sm->fetch('ships_in/ed_ship_page'.$print_add.'.html');
