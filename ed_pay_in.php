...
require_once('classes/payforbill_in_group.php');
...
require_once('classes/pay_in_creator.php');
...
$smarty->assign("SITETITLE",'Редактирование входящей оплаты');
...
if($print!=0){
	if(!$au->user_rights->CheckAccess('w',693)){
	  header("HTTP/1.1 403 Forbidden");
	  header("Status: 403 Forbidden");
	  include("403.php");
	  die();	
...
if($action==0){
	...
	$editing_user=$_pay->GetItemByFields(array('id'=>$id,'is_incoming'=>1));
...
if($action==1){
	$log=new ActionLog;
	if($print==0)
	$log->PutEntry($result['id'],'открыл карту входящей оплаты',NULL,683, NULL, $editing_user['code'],$id);
	else
	$log->PutEntry($result['id'],'открыл карту входящей оплаты: версия для печати',NULL,693, NULL, $editing_user['code'],$id);
...
if(($action==0)&&(isset($_POST['doNew'])||isset($_POST['doNewEdit']))){
	...
	//запись в журнале
	if($code>0){
		$log->PutEntry($result['id'],'создал входящую оплату',NULL,93,NULL,NULL,$bill_id);	
		
		$log->PutEntry($result['id'],'создал входящую оплату',NULL,679,NULL,NULL,$code);
		...
		foreach($params as $k=>$v){
				...
				if($k=='supplier_id'){
					...
					$log->PutEntry($result['id'],'создал входящую оплату',NULL,679, NULL, SecStr('установлен контрагент '.$si['code'].' '.$opf['name'].' '.$si['full_name']),$code);			
					continue;	
				}
				
				
				$log->PutEntry($result['id'],'создал входящую оплату',NULL,679, NULL, 'в поле '.$k.' установлено значение '.$v,$code);		
	...
	if(isset($_POST['doNew'])){
		if($bill_id!=0) header("Location: ed_bill.php?action=1&id=".$bill_id.'&do_show_pay=1');
		...
		if(!$au->user_rights->CheckAccess('w',683)){
			header("HTTP/1.1 403 Forbidden");
			header("Status: 403 Forbidden");
			include("403.php");
			die();	
		}
		header("Location: ed_pay_in.php?action=1&id=".$code);
		die();	
	...
	if($condition){	
		...
		foreach($params as $k=>$v){
			...
			if(addslashes($editing_user[$k])!=$v){
				...
				if($k=='given_pdate'){
					$log->PutEntry($result['id'],'редактировал заданную дату входящей оплаты',NULL,93,NULL,'дата: '.$_POST['given_pdate'],$bill_id);
					
					$log->PutEntry($result['id'],'редактировал заданную дату',NULL,683,NULL,'дата: '.$_POST['given_pdate'],$id);
					continue;	
				}
				
				$log->PutEntry($result['id'],'редактировал входящую оплату',NULL,93, NULL, 'в поле '.$k.' установлено значение '.$v,$bill_id);
				
				$log->PutEntry($result['id'],'редактировал входящую оплату',NULL,683, NULL, 'в поле '.$k.' установлено значение '.$v,$id);
			}
			
		}
		
		
		if($au->user_rights->CheckAccess('w',681)){
		  $positions=array();
			  ...
			  if($v['action']==0){
				  if($v['kind']==0){
					 ...
					 if($pos!==false){
						...
						$log->PutEntry($result['id'],'добавил счет к входящей оплате', NULL, 93,NULL,$descr,$pos['id']);	
					
						$log->PutEntry($result['id'],'добавил счет к входящей оплате', NULL, 681,NULL,$descr,$id);	
					 }
				  }else{
					 ...
					 if($pos!==false) {
						...
						$log->PutEntry($result['id'],'добавил инвентаризационный акт к оплате', NULL, 681,NULL,$descr,$id);	  
					 }
				  }
				   ...
				   if($v['kind']==0){
					 ...
					 if($pos!==false){
						$descr=$pos['code'].'<br /> сумма '.$v['value'].' руб.<br />';
						$log->PutEntry($result['id'],'удалил счет из входящей оплаты', NULL, 93,NULL,$descr,$pos['id']);	
					
						$log->PutEntry($result['id'],'удалил счет из входящей оплаты', NULL, 681,NULL,$descr,$id);	
					 }
				  }else{
					 $pos=$_invc->GetItemByFields(array('code'=>$v['code'])); 
					 if($pos!==false) {
						...
						$log->PutEntry($result['id'],'удалил инвентаризационный акт из оплаты', NULL, 681,NULL,$descr,$id);	  
					 }
				  }
			  }
		  }	//
	...
	if($editing_user['is_confirmed']==1){
		...
		if(($au->user_rights->CheckAccess('w',690))||$au->user_rights->CheckAccess('w',96)){
			if(!isset($_POST['is_confirmed'])&&($editing_user['status_id']==15)&&($_POST['current_status_id']==15)){
				...
				$log->PutEntry($result['id'],'снял утверждение входящей оплаты',NULL,93, NULL, NULL,$bill_id);	
				
				$log->PutEntry($result['id'],'снял утверждение входящей оплаты',NULL,690, NULL, NULL,$id);	
			}
		}else{
			//нет прав	
		}
		...
		//есть права
		if($au->user_rights->CheckAccess('w',689)||$au->user_rights->CheckAccess('w',96)){
			if(isset($_POST['is_confirmed'])&&($editing_user['status_id']==14)&&($_POST['current_status_id']==14)){
				...
				$log->PutEntry($result['id'],'утвердил входящую оплату',NULL,93, NULL, NULL,$bill_id);	
				
				$log->PutEntry($result['id'],'утвердил входящую оплату',NULL,689, NULL, NULL,$id);	
	...
	if(isset($_POST['doEdit'])){
		...
		header("Location: ed_pay_in.php?action=1&id=".$id);
		die();	
...
$_menu_id=35;
	...
	if($action==0){
		...
		$_supgroup->GetItemsForPay('pay_in/suppliers_list.html', new DBDecorator, false, $supgroup,$result);
		...
		if($bill!==false){
			...
			$scg=$_scg->GetItemsByIdArr($supplier['id'],$bill['contract_id']/*, 0*/);
			//print_r($scg);
			$sm1->assign('pos2',$scg);
		...
		$sm1->assign('can_add_positions',$au->user_rights->CheckAccess('w',680)); 
		$sm1->assign('can_del_positions',$au->user_rights->CheckAccess('w',682)); 
		
		$sm1->assign('can_create',$au->user_rights->CheckAccess('w',679)); 
		$sm1->assign('can_edit',$au->user_rights->CheckAccess('w',683)); 
		...
		$user_form=$sm1->fetch('pay_in/pay_create.html');
	}elseif($action==1){
		...
		$sm1->assign('can_add_positions',$au->user_rights->CheckAccess('w',681)); 
		$sm1->assign('can_del_positions',$au->user_rights->CheckAccess('w',682)); 
		...
		//блок аннулирования
			$editing_user['can_annul']=$_pay->DocCanAnnul($editing_user['id'],$reason)&&$au->user_rights->CheckAccess('w',691);
			if(!$au->user_rights->CheckAccess('w',691)) $reason='недостаточно прав для данной операции';
		...
		//($editing_user['is_confirmed']==0)&&($_pay->DocCanConfirm($editing_user['id'],$rss)));
		...
		if($editing_user['is_confirmed']==1){
			...
			}elseif($au->user_rights->CheckAccess('w',690)){
			...
			//95
			$can_confirm_price=$au->user_rights->CheckAccess('w',689)&&($editing_user['status_id']==14);
		}
		...
		$sm1->assign('can_print',$au->user_rights->CheckAccess('w',693)); 
		...
		$user_form=$sm1->fetch('pay_in/pay_edit'.$print_add.'.html');
		
		//покажем журнал событий по позиции
		if($au->user_rights->CheckAccess('w',694)){
			...
			$decorator->AddEntry(new SqlEntry('object_id',NULL, SqlEntry::IN_VALUES, NULL, array(677,
678,
679,
680,
681,
...
694
			...
			$llg=$log->ShowLog('syslog/log_doc.html',$decorator,$from,$to_page,'ed_pay_in.php',true,true,true);
	...
	$content=$sm->fetch('pay_in/ed_pay_page'.$print_add.'.html');
