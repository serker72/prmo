<?
/*echo date('d.m.Y H:i:s ', 1421852400);
echo 30*24*60*60; die;
*/
/*
...
echo mktime(15,17,23,02,10,2015).'<br>';
die();
...
$set=new mysqlset($sql);
			...
			?>
            <h2>Файлов реализаций загружено: <?=$rc?></h2>
            <?
			for($i=0; $i<$rc; $i++){
...
$set=new mysqlset($sql);
			...
			$rc=$set->GetResultNumRows();
			?>
            <h2>Файлов поступлений загружено: <?=$rc?></h2>
            <?
			for($i=0; $i<$rc; $i++){
				$f=mysqli_fetch_array($rs);
				$f['given_pdate']=date('d.m.Y', $f['given_pdate']);
