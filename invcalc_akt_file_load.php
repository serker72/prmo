...
$user=$_user->GetItemById($fmi['invcalc_id']);

if($fmi['folder_id']==0){
	$_folder_desrc='';
}else{
	$_ff=new FileDocFolderItem(3, $fmi['invcalc_id'], new InvCalcFileItem(3));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл инвентаризационного акта',NULL,267, NULL,SecStr('файл '.$fmi['orig_name'].' для '.  $user['code'].$_folder_desrc),$user['id']); 
