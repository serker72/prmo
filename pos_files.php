...
$smarty->assign("SITETITLE",'Файлы номенклатуры');
...
$log->PutEntry($result['id'],'открыл реестр файлов позиции номенклатуры/прайс-листа',NULL,64, NULL, $user['code'].' '.$user['name'].$_folder_desrc,$bill_id);	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил файл',NULL,68,NULL,'имя файла '.SecStr($file['orig_name']),$bill_id);
	}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил папку',NULL,69,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
