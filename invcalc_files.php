...
$log->PutEntry($result['id'],'открыл реестр файлов инвентаризационного акта',NULL,450, NULL, $user['code'].$_folder_desrc,$bill_id);	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	...
	header("Location: invcalc_files.php?bill_id=".$bill_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	$_ff=new FileDocFolderItem(4, $bill_id, new InvCalcFileItem(4));
	$_ff->SetTablename('invcalc_file_folder');
	...
	header("Location:invcalc_files.php?bill_id=".$bill_id.'&folder_id='.$folder_id);
	die();
}
...
$_menu_id=39;
	...
	*/
	...
	$ffg=new InvCalcFileGroup(4, $bill_id, new FileDocFolderItem(4,  $bill_id, new InvCalcFileItem(4)));
	
	
	$filetext=$ffg->ShowFiles('doc_file/list.html', $decorator,$from,$to_page,'invcalc_files.php', 'invcalc_akt_file.html', 'swfupl-js/invcalc_files.php', $au->user_rights->CheckAccess('w',455), $au->user_rights->CheckAccess('w',457)/*&&($user['is_confirmed_shipping']==0)*/, $au->user_rights->CheckAccess('w',456), $folder_id, $au->user_rights->CheckAccess('w',455), $au->user_rights->CheckAccess('w',456),$au->user_rights->CheckAccess('w',457),$au->user_rights->CheckAccess('w',457) ,
	
	'',
	 $au->user_rights->CheckAccess('w',819),
	 $result
	...
	//$filetext=$ffg->ShowFiles($bill_id,'invcalc/files_list.html', $decorator,$from,$to_page,'invcalc_files.php', 'invcalc_file.html', 'swfupl-js/invcalc_files.php', $au->user_rights->CheckAccess('w',455), $au->user_rights->CheckAccess('w',457)/*&&($user['is_confirmed_shipping']==0)*/, $au->user_rights->CheckAccess('w',456));
	...
	$sm->assign('bill',$user);
	$content=$sm->fetch('invcalc/files.html');
