...
$_user=new PosItem;
$user=$_user->GetItemById($fmi['position_id']);

if($fmi['folder_id']==0){
	$_folder_desrc='';
}else{
	$_ff=new FileDocFolderItem(4, $fmi['position_id'], new PosFileItem(4));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл позиции номенклатуры/прайс-листа',NULL,64, NULL,SecStr('файл '.$fmi['orig_name'].' для '.  $user['code'].' '.$user['name'].$_folder_desrc),$user['id']);
