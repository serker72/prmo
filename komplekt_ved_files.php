...
$smarty->assign("SITETITLE",'Файлы заявки');
...
$log->PutEntry($result['id'],'открыл реестр файлов заявки',NULL,82, NULL, 'заявка '.$user['id'].$_folder_desrc,$komplekt_ved_id);	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил файл',NULL,300,NULL,'имя файла '.SecStr($file['orig_name']),$komplekt_ved_id);
	}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил папку',NULL,300,NULL,'имя папки '.SecStr($file['filename']));
