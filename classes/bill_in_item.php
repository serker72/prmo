...
class BillInItem  extends BillItem{
	...
	public function AddPositions($current_id, array $positions,$can_change_cascade=false, $check_delta_summ=false, $result=NULL){
		...
		foreach($positions as $k=>$v){
			$kpi=$_kpi->GetItemByFields(array('bill_id'=>$v['bill_id'],'position_id'=>$v['position_id'],'storage_id'=>$v['storage_id'],'sector_id'=>$v['sector_id'],'komplekt_ved_id'=>$v['komplekt_ved_id'], 'out_bill_id'=>$v['out_bill_id']));
	...
	public function ScanDocStatus($id, $old_params, $new_params, $item=NULL, $_result=NULL){
		...
		if(isset($new_params['is_confirmed_price'])&&isset($old_params['is_confirmed_price'])){
			...
			if(($new_params['is_confirmed_price']==1)&&($old_params['is_confirmed_price']==0)){
				...
				$log->PutEntry($_result['id'],'смена статуса счета',NULL,613,NULL,'установлен статус '.$stat['name'],$item['id']);
	...
	/*public function ScanDocStatus($id, $old_params, $new_params, $item=NULL, $_result=NULL){
		...
		if(isset($new_params['is_confirmed_price'])&&isset($old_params['is_confirmed_price'])){
		  ...
		  //переход 2-9 утвержден - не выполнен
		  if($item['status_id']==2){
			  //проверить количество п	
			  //также может произойти переход 2-10 утвержден - выполнен
			  if($this->CheckDeltaPositions($id)){
	...
	*/
	...
	public function CheckDeltaPositions($id){
		...
		foreach($positions as $k=>$v){
			...
			where 
			acceptance_id in(
				select id from acceptance where is_confirmed=1  and bill_id="'.$v['bill_id'].'"   ) and position_id="'.$v['id'].'" and komplekt_ved_id="'.$v['komplekt_ved_id'].'" and out_bill_id="'.$v['out_bill_id'].'"';
	...
	public function DocCanAnnul($id,&$reason,$item=NULL){
		...
		if($item['status_id']!=1){
		  ...
		  for($i=0; $i<$rc; $i++){
			  ...
			  //if($v['is_confirmed']==1) {
				  $can=$can&&false;
				  $reasons[]=' поступление <a target=_blank href=ed_acc_in.php?id='.$v['id'].'&action=1&from_begin=1>№ '.$v['id'].'</a> статус документа: '.$v['name'];	
			  //}
	...
	public function GetBindedDocumentsToAnnul($id){
		...
		//проверить связанные поступления
		 ...
		 $set=new mysqlSet('select p.*, s.name from acceptance as p left join document_status as s on p.status_id=s.id where p.status_id=4  and bill_id="'.$id.'"');
		  $rs=$set->getResult();
		  $rc=$set->GetResultNumRows();
		  $reasons=array();
		  for($i=0; $i<$rc; $i++){
	...
	public function AnnulBindedDocuments($id){
		...
		for($i=0; $i<$rc; $i++){
			...
			$log->PutEntry($_result['id'],'аннулирование поступления в связи с аннулированием счета',NULL,626,NULL,'поступление № '.$f['id'].': установлен статус '.$stat['name'],$f['bill_id']);
			
			$log->PutEntry($_result['id'],'аннулирование поступления в связи с аннулированием счета',NULL,674,NULL,'поступление № '.$f['id'].': установлен статус '.$stat['name'],$f['id']);
		}	
	...
	public function GetBindedPaymentsFull($bill_id){
		...
		$sql='select distinct pb.payment_id, "0" as kind, pb.id, b.code as bill_code, p.code, p.value, p.given_no, p.given_pdate as given_pdate,  p.given_pdate as given_payment_pdate, p.given_pdate as given_payment_pdate_unf 
					 from payment_for_bill as pb 
						inner join payment as p on p.id=pb.payment_id
						inner join bill as b on pb.bill_id=b.id
		where pb.bill_id="'.$bill_id.'" and p.is_confirmed=1 order by p.given_pdate desc';
	...
	public function CanConfirmByPositions($id,&$reason,$item=NULL){
		...
		foreach($positions as $k=>$v){
			if(!in_array($v['out_bill_id'], $komplekt_ved_ids)) $komplekt_ved_ids[]=$v['out_bill_id'];	
		}
		...
		foreach($positions as $k=>$v){
			...
			if(!$this->PosInSh($v,$ship_positions,$find_pos)){
				$can=$can&&false;
				$reasons[]='в родительском исходящем счете № '.$v['out_bill_id'].' не найдена позиция '.SecStr($v['position_name'].', выберите другую позицию, нажав кнопку Редактировать позиции..., затем сохраните счет и повторите попытку');	
				continue;	
			}
			...
			$free=$mf->MaxForIncomingBill($v['out_bill_id'], $v['id'], $v['komplekt_ved_id'],$id); //>MaxForBill($v['komplekt_ved_id'],$v['id']) ;
			
						
			if($v['quantity']>$free*PPUP){
				//превышение
				$can=$can&&false;
				$reasons[]='количество позиции '.SecStr($v['position_name']).' '.$v['quantity'].' '.SecStr($v['dim_name']).', заявка №'.$v['komplekt_ved_id'].', превышает доступное количество по исходящему счету ('.round($free*PPUP,3).'  '.SecStr($v['dim_name']).')';	
				continue;		
			}
	...
	//есть ли данная позиция в род. счете. есть - вернуть поз. счета, нет - false
	protected function PosInSh($acc_position, $sh_positions, &$find_pos){
		$has=false;
		$find_pos=NULL;
		foreach($sh_positions as $k=>$v){
			if(($v['position_id']==$acc_position['position_id'])&&($v['komplekt_ved_id']==$acc_position['komplekt_ved_id'])){
				$has=true;
				$find_pos=$v;
				break;	
			}
	...
	//проверка, превышают ли цены в счете цены в исхю счете
	public function CanConfirmByPricePositions($id,$positions=NULL, &$reason,$item=NULL){
		$reason=''; $reasons=array();
		$can=true;	
		...
		$_sh=new BillItem;
		$_bpm=new  BillPosItem;
		
		
		// если явно передали позиции - проверяем то, что явно передано
		//иначе подгружаем все позиции
		if($positions===NULL) $positions=$this->GetPositionsArr($id);
		//
		
		foreach($positions as $k=>$v){
			if($v['komplekt_ved_id']==0) continue; //услуши пропускаем
			
			
			//найти позицию родительского счета, сравнить цену_пм
			$bpm=$_bpm->getitembyfields(array('komplekt_ved_id'=>$v['komplekt_ved_id'], 'position_id'=>$v['position_id'], 'bill_id'=>$v['out_bill_id']));
			
			if($bpm!==false){
				if($v['price_pm']>$bpm['price_pm']){
					$can=$can&&false;
					
					$bill=$_sh->Getitembyid($v['out_bill_id']);
					
					$reasons[]='цена позиции '.SecStr($bpm['name']).' в родительском исходящем счете № '.$bill['code'].' '.$bpm['price_pm'].' руб. меньше цены во входящем счете '.$v['price_pm'].' руб.';	
					
				//	echo $v['price_pm'].' vs '.$bpm['price_pm'].' ';
				}
	...
	public function DoEq($id, array $args, &$output, $is_auto=0, $sh=NULL, $_result=NULL, $express_scan=false, $extra_reason=''){
		...
		//находим все расп. на приемку, ровняем их
		if($is_auto==0){
		...
		foreach($items as $k=>$v){
			...
			$sh_p=$_sh_p->GetItemByFields(array('bill_id'=>$id, 'storage_id'=>0/*$v['storage_id']*/, 'sector_id'=>0/*$v['sector_id']*/, 'position_id'=>$v['position_id'],'komplekt_ved_id'=>$v['komplekt_ved_id'], 'out_bill_id'=>$v['out_bill_id']));
			...
			if($sh_p!==false){
				...
				if($v['delta']>=0){ //выравнивать только недовоз! перевоз не выравнивать!
				 ...
				 // $description=$sh_p['name'].' <br /> Кол-во: '.$params['quantity'].'<br /> ';
				  ...
				  //создать примечание 
				  if($is_auto==1){
					 $log->PutEntry(0,'автоматическое редактирование позиции счета в связи с выравниванием позиций',NULL,613,NULL,SecStr($description.$extra_reason),$id);	 
					 $posted_user_id=0;
					 ...
					 $log->PutEntry($_result['id'],'редактировал позицию счета в связи с выравниванием позиций',NULL,613,NULL,SecStr($description.$extra_reason),$id);	
	...
	public function ScanEq($id, array $args, &$output, $sh=NULL, $express_scan=false, $continue_message=".\nПродолжить выравнивание данной позиции?"){
		...
		foreach($args as $k=>$v){
			...
			$sql='select * from acceptance_position where acceptance_id in(select id from acceptance where is_confirmed=1 and bill_id="'.$id.'" ) and position_id="'.$_t_arr[0].'" and komplekt_ved_id="'.$_t_arr[4].'" and out_bill_id="'.$_t_arr[5].'"';
			...
			for($i=0; $i<$rc; $i++){
				...
				if(!in_array('№'.$f['acceptance_id'],$docs)){
					 ...
					 $docs_ext[]='<a href="ed_acc_in.php?action=1&id='.$f['acceptance_id'].'" target="_blank">'.'№'.$f['acceptance_id'].'</a>';
				}
			}
			...
			$items[]=array('position_id'=>$_t_arr[0], 'quantity'=>$_t_arr[1], 'storage_id'=>$_t_arr[2], 'sector_id'=>$_t_arr[3], 'komplekt_ved_id'=>$_t_arr[4], 'delta'=>round(($_t_arr[1]-$summ),3), 'out_bill_id'=>$_t_arr[5]);
		...
		if($total_summ==0){
			...
			//в поступлениях 
			if(count($docs)){
				 $output.="<li>\nПозиция ".htmlspecialchars($pos["name"])." найдена в утвержденных поступлениях: ".implode(", ",$docs_ext)." в количестве ".$count_acc." ".htmlspecialchars($pdi["name"]);
