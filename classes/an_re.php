...
class AnRe{
	...
	*/
	...
	public function ShowData($pdate1, $pdate2,  $supplier_name='', $template, DBDecorator $dec,$pagename='files.php', $do_show_data=false, $can_print=false, $dec_sep=DEC_SEP, &$alls, $limited_supplier=NULL, &$total_pribyl){
		...
		if($do_show_data){
			...
			where 
				...
				b.id in(select distinct bill_id from acceptance where is_incoming=0 and is_confirmed=1 and  given_pdate between "'.$pdate1.'" and "'.$pdate2.'")
			...
			for($i=0; $i<$rc; $i++){
				...
				where
					...
					and  a.given_pdate between "'.$pdate1.'" and "'.$pdate2.'"
				group by 
					ap.position_id	
					';
				...
				where
					...
					and (ap.out_bill_id="'.$f['out_bill_id'].'" or (ap.out_bill_id=0 and a.out_bill_id="'.$f['out_bill_id'].'"))
					and ap.position_id="'.$f['position_id'].'"
				...
				//if($f['name']=='Доставка') echo $f['out_ap_total'].'<br>';
				...
				for($i1=0; $i1<$rc1; $i1++){
					...
					//if($f1['name']=='Доставка') echo $f1['acceptance_id'].'<br>';
