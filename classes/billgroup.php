...
// группа исх счетов
class BillGroup extends AbstractBillGroup {
			...
			if($f['is_confirmed_price']==0){
				...
				if($f['restore_pdate']>0){
					...
					if(($f['pdate']+$days*24*60*60)>$now){
						...
						$can_annul=true;
						$reason='прошло более '.$days.' дней с даты создания исходящего счета, нет утвержденных связанных документов, документ не утвержден';
					}
				}
			...
			if($can_annul){
				...
				for($j=0; $j<$rc2; $j++){
					...
					if($can_annul_position){
						...
						$args[]=$h['position_id'].';'.$h['quantity'].';0;'.$h['sector_id'].';'.$h['komplekt_ved_id'];	
