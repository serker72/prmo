...
class AnRent{
	...
	*/
	...
	function __construct( $begin_ost=200000, $begin_pdate='28.02.2014'){
	...
	public function ShowData($pdate1, $pdate2, $extended_an=0, $org_id, $template, DBDecorator $dec,$pagename='files.php', $do_show_data=false, $can_print=false, $dec_sep=DEC_SEP){
		...
		if($do_show_data){
			...
			$sql='select sum(ap.total) 
				...
				where ac.is_confirmed=1 and ac.is_incoming=1 and ac.org_id="'.$org_id.'"
			...
			$sql='select sum(ap.total) 
				...
				where ac.is_confirmed=1 and ac.is_incoming=0 and ac.org_id="'.$org_id.'"
					and ( ac.given_pdate between "'.$pdate1.'" and  "'.($pdate2+24*60*60-1).'" )
					';
			...
			foreach($pcg as $k=>$v){
				...
				$sql='select sum(p.value) 
					from payment as p
					where p.is_confirmed=1 and p.is_incoming=0 and p.org_id="'.$org_id.'"
			...
			if($extended_an){
					...
					foreach($orgs_docs1 as $k=>$rec){
						...
						foreach($rec['docs_on_date'] as $kk=>$doc){
							...
							if(($doc['kind']==1)||($doc['kind']==5)){
								$bbo=$bbo+$doc['value'];
							}else $bbo=$bbo-$doc['value'];
					...
					*/
					/*
					$docs=$this->GetDocs($pdate1, $pdate2, $org_id);
					...
					*/
					...
					//и еще куда-то внести остаток по организации!!!!
	...
	public function GetDocsAcc($pdate1, $pdate2, $org_id){
		...
		select "0" as kind, 
			...
			where
				a.is_incoming=0 and a.is_confirmed=1 and a.org_id="'.$org_id.'"
		...
		select "1" as kind, 
			...
			where
				a.is_incoming=1 and a.is_confirmed=1 and a.org_id="'.$org_id.'"
				and (a.given_pdate  between "'.$pdate1.'" and  "'.($pdate2+24*60*60-1).'" )
		)
	...
	/*public function GetDocs($pdate1, $pdate2, $org_id){
		$_ai=new AccItem;
		$_ai_in=new AccInItem;
		
		$alls=array();
		$sql='
			(select p.id, p.id as code, 0 as value, p.given_pdate, p.given_no,
			"0" as kind,
			...
			where 
				p.is_confirmed=1 and p.is_incoming=1 and p.org_id="'.$org_id.'"
				and ( p.given_pdate between "'.$pdate1.'" and  "'.($pdate2+24*60*60-1).'" )
			...
			where 
				p.is_confirmed=1 and p.is_incoming=0 and p.org_id="'.$org_id.'"
				and ( p.given_pdate between "'.$pdate1.'" and  "'.($pdate2+24*60*60-1).'" )					
			...
			"4" as kind,
				...
				from cash as p
				left join payment_code as pc on pc.id=37
				left join supplier as sp on p.supplier_id=sp.id
			   left join opf on sp.opf_id=opf.id
		...
		//включим входящий остаток в список документов ! если его дата попадает в наш интервал
		if(($this->begin_pdate>=$pdate1)&&($this->begin_pdate<=($pdate2+24*60*60-1))){
			...
			$alls[]=array(
				'kind'=>5,
				'value'=>$this->begin_ost,
				'given_pdate'=>date('d.m.Y', $this->begin_pdate)
	...
	//список поступлений, реализаций за период
	/*public function GetDocsAcc($pdate1, $pdate2, $org_id){
		$_ai=new AccItem;
		$_ai_in=new AccInItem;
	...
	public function GetDocsPayCash($pdate1, $pdate2, $org_id, $code_id){
		...
		$sql='
			...
			where p.is_confirmed_given=1 and p.org_id="'.$org_id.'"
				and (p.confirmed_given_pdate  between "'.$pdate1.'" and "'.($pdate2+24*60*60-1).'" )
				and p.code_id ="'.$code_id.'" and p.code_id ="37"	
