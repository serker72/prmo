...
// абстрактная группа
class InvoicePosGroup extends AbstractGroup {
	...
	public function GetItemsByIdArr($id, $current_id=0,$has_goods=true,$has_usl=true, $show_statiscits=true, $show_boundaries=true){
		...
		$sql='select p.id as p_id, p.invoice_id, p.position_id as id,
		...
		for($i=0; $i<$rc; $i++){
			...
			$f['hash']=md5($f['id'].'_'.$f['komplekt_ved_id'].'_'.$f['invoice_in_id']);
			...
			if(($ac['inventory_id']==0)&&($ac['interstore_id']==0)){
				if($show_statiscits){
				  ...
				  $f['max_incoming_quantity']=$_mf->MaxForAccByAccIn($ac['bill_id'], $f['id'], $id, NULL, $ac['storage_id'],$ac['sector_id'], $f['komplekt_ved_id'], $f['invoice_in_id']);
