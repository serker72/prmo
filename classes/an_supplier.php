...
class AnSupplier{
	...
	public function ShowData($supplier_name, $org_id, $pdate1, $pdate2, $extended_an=0, $template, DBDecorator $dec,$pagename='files.php', $do_show_data=false, $can_print=false, $dec_sep=DEC_SEP, $similar_firms=0, $by_contract=0, $limited_supplier=NULL, $can_view_all_pays=false, $can_print_sverka=false ){
		...
		*/
		...
		if(strlen($supplier_name)>0){
			...
			*/
			...
			if($similar_firms==0){
				...
				$supplier_ids=explode(';',$supplier_name);	
				
				 
				$sql='select * from supplier where   is_active=1 and ((is_org=0 and org_id="'.$org_id.'") or (is_org=1 and id<>"'.$org_id.'")) and  id in(';
				
				$_supplier_names=array();
				foreach($supplier_ids as $k=>$v){
					
					if(strpos($v, 'U')!==false){
						continue; //пропустим пользователей
					}
					if(strlen(trim($v))>0){
						//if($similar_firms==0) $_supplier_names[]=' (BINARY full_name ="'.trim($v).'") ';
						//else $_supplier_names[]=' (full_name LIKE "%'.trim($v).'%") ';
				...
				//$sql.= implode(' OR ',$_supplier_names).') order by full_name desc';
				$sql.= implode(', ',$_supplier_names).') order by full_name asc';
				...
				$sql='select * from supplier where  is_active=1 and ((is_org=0 and org_id="'.$org_id.'") or (is_org=1 and id<>"'.$org_id.'")) and (';
				
				$_supplier_names=array();
				foreach($supplier_ids as $k=>$v){
					 
					if(strlen(trim($v))>0){
						 $_supplier_names[]=' (full_name LIKE "%'.trim($v).'%") ';
						 
						
					}
				}
				
				$sql.= implode(' OR ',$_supplier_names).') order by full_name asc';
			}
			...
			*/
			...
			if($similar_firms==0){
				...
				$sql='select * from user where is_active=1  and id in (';
				
				$_supplier_names=array();
				foreach($supplier_ids as $k=>$v){
					
					if(strpos($v, 'U')===false){
						continue; //пропустим контрагентов
					}else $v=str_replace('U','',$v);
					if(strlen(trim($v))>0){
						 $_supplier_names[]=trim($v);
				 ...
				 $sql.= implode(', ',$_supplier_names).') order by name_s asc';
				...
				foreach($supplier_ids as $k=>$v){
					...
					if(strlen(trim($v))>0){
						 $_supplier_names[]=' (name_s LIKE "%'.trim($v).'%") ';
		...
		$sri_1=$_sri->GetActual($org['id'], 1);
		$sri_2=$_sri->GetActual($org['id'],  2);
		  ...
		  foreach($sg as $k=>$v){
			 ...
			 // if(($v['id']==2)&&($v['supplier_or_user']==1))  var_dump( ($_dog_arr));
			  ...
			  if(($v['supplier_or_user']==1)&&in_array($v['id'],$user_id)&&($supplier_dolg==0)&&($supplier_plus==0)&&($v['before_ost_unf']==0)&&($v['after_ost_unf']==0)){
			  	...
			  	//$_sri=new SupplierRukItem;
		$sri_1=$_sri->GetActual($v['id'], 1);
		$sri_2=$_sri->GetActual($v['id'], 2);
		
		
		$v['chief']=$sri_1['fio'];
		$v['print_sign_dir']=$sri_1['sign'];
		
		$v['main_accountant']=$sri_2['fio'];
		$v['print_sign_buh']=$sri_2['sign'];
		...
		//подставим в поля ввода выбранные данные
		$_sqls=array();
		if(count($supplier_id)>0) $_sqls[]='(select p.id, p.full_name as full_name,
		...
		"" as login
		 ...
		 where   p.is_active=1 and p.id in('.implode(', ', $supplier_id).'))';
		 
		 if(count($user_id)>0) $_sqls[]='(select p.id, p.name_s as full_name,
		"" as opf_name,
		"" as code,
		p.login as login
		
		 from user as p where p.is_active=1 and  p.id in('.implode(', ', $user_id).'))';
		 
		$sql=implode( ' UNION distinct ',$_sqls);
		if(strlen($sql)>0) $sql.=' order by 2 asc';
		...
		for($i=0; $i<$rc; $i++){
			...
			if($f['code']==""){
				 $f['full_name']= $f['login'].' '.$f['full_name'];
				 $f['id']='U'.$f['id'];
			}else $f['full_name']=$f['code'].' '.$f['full_name'].', '.$f['opf_name'];
			
			
			$our_suppliers[]=$f;
		}
		$sm->assign('our_suppliers', $our_suppliers);
		//var_dump ($our_suppliers);
		...
		$sm->assign('can_print_sverka', $can_print_sverka);	
	...
	public function OstBySup($supplier_id, $pdate, $org_id, $except_invcalc_id=NULL, $supplier_or_user=0){
		...
		/*
		//1
		$sql='select sum(value) as value  from payment
		...
		$before_ost+=(float)$g['value'];
		
		//2
		$sql='select   sum(ap.total)  as value
		...
		//7
		$sql='select sum(value) as value from demand
