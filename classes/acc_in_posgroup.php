...
class AccInPosGroup extends AbstractGroup {
	...
	protected function init(){
		...
		if(self::$semi_uslugi===NULL){
		 ...
		 // var_dump(self::$semi_uslugi);
		// echo SEMI_SERVICE_CODE;
		}
	...
	public function GetItemsByIdArr($id, $current_id=0,$has_goods=true,$has_usl=true, $show_statiscits=true, $show_boundaries=true){
		$arr=Array();
		 ...
		 $acc_in=$_acc_in->GetItemById($id);
		 
		 $_bill_in=new BillInItem;
		 $bill=$_bill_in->getitembyid($acc_in['bill_id']);
		...
		$sql='select p.id as p_id, p.acceptance_id, p.komplekt_ved_pos_id, p.position_id as id,
					 ...
					 cat.group_id,
					 p.out_bill_id, oc.code as out_bill_code
		from '.$this->tablename.' as p 
			...
			left join bill as oc on oc.id=p.out_bill_id
		...
		for($i=0; $i<$rc; $i++){
			 ...
			 //	echo ' zzzzZZZ ';
			  $f['in_bill']=round($_mf->MaxInBill($ac['bill_id'], $f['id'],$ac['storage_id'],$ac['sector_id'],$f['komplekt_ved_id']),3);
				 // $f['in_rasp']=round($_mf->MaxInShI($ac['bill_id'], $f['id'],$ac['sh_i_id'],$ac['storage_id'],$ac['sector_id'],$f['komplekt_ved_id']),3);
				  $f['in_acc']=round($_mf->MaxInInAcc($ac['bill_id'], $f['id'], $id,$ac['sh_i_id'],$ac['storage_id'],$ac['sector_id'],$f['komplekt_ved_id']),3);
			}
			...
			if(($ac['inventory_id']==0)&&($ac['interstore_id']==0)){
				if($show_statiscits){
				  ...
				  $f['max_komplekt_quantity']=$_mf->MaxForIncomingBill($f['out_bill_id'],  $f['id'], $f['komplekt_ved_id'], $ac['bill_id']);//MaxInKomplekt($f['komplekt_ved_id'],$f['id']);
				}
