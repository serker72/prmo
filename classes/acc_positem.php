...
//абстрактный элемент
class AccPosItem extends AbstractItem{
...
//добавить 
	public function Add($params, $pms=NULL,$change_high_mode=0,$change_low_mode=0){
		...
		/*if(isset($params['acceptance_id'])&&isset($params['position_id'])&&isset($params['quantity'])&&isset($params['komplekt_ved_id'])) 
			$this->SetChainQuantity($params['acceptance_id'],$params['position_id'], $params['komplekt_ved_id'],$params['quantity'],$change_high_mode,$change_low_mode);
	...
	public function SetChainQuantity($acceptance_id,$position_id, $komplekt_ved_id, $quantity,$change_high_mode=0,$change_low_mode=0, $_result=NULL, $item=NULL){
		...
		if($ai!==false){
			...
			if($bpi!==false){
				...
				$set=new MysqlSet('select sum(quantity) from acceptance_position where acceptance_id<>"'.$acceptance_id.'" and position_id="'.$position_id.'" and komplekt_ved_id="'.$komplekt_ved_id.'" and acceptance_id in(select id from acceptance where bill_id="'.$bpi['bill_id'].'" and is_confirmed=1)');
				$rs=$set->getResult();
				...
				(($delta_rasp<0)&&($change_low_mode==1))*/
					...
					//  echo 'ищем позицию род.счета:'; var_dump(array('bill_id'=>$ai['bill_id'], 'position_id'=>$position_id, 'storage_id'=>0/*$ai['storage_id']*/, 'sector_id'=>0/*$ai['sector_id']*/, 'komplekt_ved_id'=>$komplekt_ved_id)); die();
					  ...
					  if($bill_positem!==false){
						  ...
						  $note='Количество позиции '.SecStr($bill_positem['name']).' автоматически выровнено при утверждении связанной реализации №'.$acceptance_id.' сотрудником '.$_result['name_s'].' ('.$_result['login'].')'.'. Первичное количество '.$bill_positem['quantity'].', конечное количество '.$new_bill_quantity;
