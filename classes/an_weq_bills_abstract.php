...
// 
class AnWeqBillsAbstract extends AbstractGroup {
	...
	public function ShowPos($template, DBDecorator $dec,$from=0,$to_page=ITEMS_PER_PAGE, $can_add=false, $can_edit=false, $can_delete=false, $add_to_bill='', $can_confirm=false,  $can_super_confirm=false, $has_header=true, $is_ajax=false, $can_restore=false,$limited_sector=NULL,$nested_bill_positions=NULL, $can_confirm_ship=false, $can_unconfirm=false, $can_unconfirm_ship=false, $can_eq, $do_show=false){
		...
		$sql='select p.*,
				...
				from '.$this->tablename.' as p
					...
					left join user as utv on p.cannot_eq_id=utv.id
		...
		for($i=0; $i<$rc; $i++){
			...
			if($f['cannot_eq_pdate']==0){
				$f['utv_pdate']=' ';	
			}else $f['utv_pdate']=date("d.m.Y H:i:s",$f['cannot_eq_pdate']);
