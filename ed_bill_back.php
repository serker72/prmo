	...
	if($editing_user['is_confirmed_shipping']==0){
	  if($editing_user['is_confirmed_price']==1){
		  ...
		  if(($au->user_rights->CheckAccess('w',196))||$au->user_rights->CheckAccess('w',96)){
			  if((!isset($_POST['is_confirmed_price']))&&in_array($editing_user['status_id'], array(2,9,10,20,21))&&in_array($_POST['current_status_id'], array(2,9,10,20,21))){
	...
	if($editing_user['is_confirmed_price']==1){
	  if($editing_user['is_confirmed_shipping']==1){
		  ...
		  if(($au->user_rights->CheckAccess('w',197))||$au->user_rights->CheckAccess('w',96)){
			  ...
			  if((!isset($_POST['is_confirmed_shipping'])) &&in_array($editing_user['status_id'], array(2,9,10,20,21))&&in_array($_POST['current_status_id'], array(2,9,10,20,21))){
...
unset($smarty);
 ...
 Пожалуйста, нажмите кнопку "Создать исходящий счет и перейти к утверждению" на вкладке "Исходящий счет" для получения возможности просмотра журнала событий.');		
	...
	/*	$_supgroup->GetItemsForBill('bills/suppliers_list.html', new DBDecorator, false, $supgroup, $result); //GetItemsByFieldsArr(array('org_id'=>$result['org_id'],'is_org'=>0,'is_active'=>1));
		...
		*/
		...
		if($print==1){
				...
				$cter=1;
				foreach($bpg as $k=>$v){
					if($cter==$number_per_page) $bpg[$k]['break_after']=true;
					elseif(($cter>$number_per_page)&&((($cter-$number_per_page)%56)==0))  $bpg[$k]['break_after']=true;
	...
	//	$summa_propis=mb_convert_case(substr($summa_propis, 1,1), MB_CASE_UPPER, 'windows-1251').substr($summa_propis, 2,strlen($summa_propis));
		...
		if(($editing_user['is_confirmed_price']==1)&&($editing_user['user_confirm_price_id']!=0)){
			...
			//$confirmer=$_user_confirmer['position_s'].' '.$_user_confirmer['name_s'].' '.' '.$_user_confirmer['login'].' '.date("d.m.Y H:i:s",$editing_user['active_pdate']);
		...
		if(($editing_user['is_confirmed_shipping']==1)&&($editing_user['user_confirm_shipping_id']!=0)){
			...
			$confirmer=$_user_confirmer['position_s'].' '.$_user_confirmer['name_s'].' '.' '.$_user_confirmer['login'].' '.date("d.m.Y H:i:s",$editing_user['confirm_shipping_pdate']);
		...
		if($editing_user['is_confirmed_price']==1){
			...
			if($au->user_rights->CheckAccess('w',678)){
			  ...
			  if(isset($_GET['pay_status_id'])){
				  if($_GET['pay_status_id']>0){
					  $dec2->AddEntry(new SqlEntry('p.status_id',abs((int)$_GET['pay_status_id']), SqlEntry::E));
				  }
				  $dec2->AddEntry(new UriEntry('pay_status_id',$_GET['pay_status_id']));
			  }else{
			  
				if(isset($_COOKIE['pay_status_id'])){
						$pay_status_id=$_COOKIE['pay_status_id'];
				}else $pay_status_id=0;
				
				if($pay_status_id>0) $dec2->AddEntry(new SqlEntry('p.status_id',$pay_status_id, SqlEntry::E));
				$dec2->AddEntry(new UriEntry('pay_status_id',$pay_status_id));
			  }
 ...
 Пожалуйста, проставьте обе галочки утверждения: "Утверждаю цену"  и нажмите кнопку "Сохранить и остаться", затем "Утверждаю отгрузку"  и нажмите кнопку "Сохранить и остаться" на вкладке "Исходящий счет" для получения возможности просмотра затрат.'); 
		...
		if($editing_user['is_confirmed_price']==1){
			...
			if(isset($_GET['bill_status_id_in_bill'])){
				if($_GET['bill_status_id_in_bill']>0){
					$decorator->AddEntry(new SqlEntry('p.status_id',abs((int)$_GET['bill_status_id_in_bill']), SqlEntry::E));
				}
				$decorator->AddEntry(new UriEntry('bill_status_id',$_GET['bill_status_id_in_bill']));
			}else{
			
			  if(isset($_COOKIE['bill_status_id_in_bill'])){
					  $bill_status_id=$_COOKIE['bill_status_id_in_bill'];
			  }else $bill_status_id=0;
			  
			  if($status_id>0) $decorator->AddEntry(new SqlEntry('p.status_id',$bill_status_id, SqlEntry::E));
			  $decorator->AddEntry(new UriEntry('bill_status_id_in_bill',$bill_status_id));
			}
 ...
 Пожалуйста, проставьте галочку "Утверждаю цену" и нажмите кнопку "Сохранить и остаться" на вкладке "Исходящий счет" для получения возможности просмотра входящих счетов.'); 	
		...
		if($editing_user['is_confirmed_shipping']==1){
			...
			if($au->user_rights->CheckAccess('w',365)){
				...
				foreach($ug as $k=>$v){
					$_ids[]=$v['id']; $_vals[]=$v['name_s'].' '.$v['login'].' '.$v['position_s'];	
				}
...
860
			...
			$llg=$log->ShowLog('syslog/log.html',$decorator,$from,$to_page,'ed_bill.php',true,true,true);
