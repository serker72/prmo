...
$smarty->assign("SITETITLE",'Файлы исходящего счета');
...
if(!$au->user_rights->CheckAccess('w',97)){ //192)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if($folder_id==0){
	...
	$_ff=new FileDocFolderItem(4, $bill_id, new BillFileItem(4));
...
$log->PutEntry($result['id'],'открыл реестр файлов исходящего счета',NULL,97, NULL, $user['code'].$_folder_desrc,$bill_id);	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	if(!$au->user_rights->CheckAccess('w',194)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	...
	$_file=new BillFileItem;
	$file=$_file->GetItemById($id);
	
	if($file!==false){
		$_file->Del($id);
		
		$log->PutEntry($result['id'],'удалил файл',NULL,194,NULL,'имя файла '.SecStr($file['orig_name']),$bill_id);
	}
	
	header("Location: bill_files.php?bill_id=".$bill_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		$_ff->Del($id,$file,$result,194);
		
		$log->PutEntry($result['id'],'удалил папку',NULL,194,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
	
	header("Location:bill_files.php?bill_id=".$bill_id.'&folder_id='.$folder_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==4)){
	...
	$_ff->SetDocIdName('bill_id');
		    ...
		    $_ff->Edit($v, array('parent_id'=>$move_folder_id), NULL,$result, 194);
...
$_menu_id=15;
	...
	*/
	...
	$ffg=new BillFileGroup(4, $bill_id,  new FileDocFolderItem(4,  $bill_id, new BillFileItem(4)));
	
	$filetext=$ffg->ShowFiles('doc_file/list.html', $decorator,$from,$to_page,'bill_files.php', 'bill_file.html', 'swfupl-js/bill_files.php', $au->user_rights->CheckAccess('w',192), $au->user_rights->CheckAccess('w',194)/*&&($user['is_confirmed_shipping']==0)*/, $au->user_rights->CheckAccess('w',193), $folder_id, $au->user_rights->CheckAccess('w',192), $au->user_rights->CheckAccess('w',193), $au->user_rights->CheckAccess('w',194), $au->user_rights->CheckAccess('w',194),
	...
	//$ffg=new BillFileGroup(4);
	
	//$filetext=$ffg->ShowFiles($bill_id,'bills/files_list.html', $decorator,$from,$to_page,'bill_files.php', 'bill_file.html', 'swfupl-js/bill_files.php', $au->user_rights->CheckAccess('w',192), $au->user_rights->CheckAccess('w',194)/*&&($user['is_confirmed_shipping']==0)*/, $au->user_rights->CheckAccess('w',193));
	...
	$sm->assign('bill',$user);
	$content=$sm->fetch('bills/files.html');
