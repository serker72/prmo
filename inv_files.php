...
require_once('classes/invfileitem.php');
...
if(!$au->user_rights->CheckAccess('w',321)){ //192)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	if(!$au->user_rights->CheckAccess('w',330)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	...
	$_file=new InvFileItem;
	$file=$_file->GetItemById($id);
	
	if($file!==false){
		$_file->Del($id);
		
		$log->PutEntry($result['id'],'удалил файл',NULL,330,NULL,'имя файла '.SecStr($file['orig_name']),$bill_id);
	}
	
	header("Location: inv_files.php?bill_id=".$bill_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	$_ff=new FileDocFolderItem(4, $bill_id, new InvFileItem(4));
	$_ff->SetTablename('inventory_file_folder');
	...
	if($file!==false){
		$_ff->Del($id,$file,$result,330);
		
		$log->PutEntry($result['id'],'удалил папку',NULL,330,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
	
	header("Location:inv_files.php?bill_id=".$bill_id.'&folder_id='.$folder_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==4)){
	...
	$_ff->SetDocIdName('bill_id');
		    ...
		    $_ff->Edit($v, array('parent_id'=>$move_folder_id), NULL,$result, 330);
...
$_menu_id=39;
	...
	*/
	...
	$ffg=new InvFileGroup(4, $bill_id, new FileDocFolderItem(4,  $bill_id, new InvFileItem(4)));
	
	//$filetext=$ffg->ShowFiles($bill_id,'inv/files_list.html', $decorator,$from,$to_page,'inv_files.php', 'inv_file.html', 'swfupl-js/inv_files.php', $au->user_rights->CheckAccess('w',328), $au->user_rights->CheckAccess('w',330)/*&&($user['is_confirmed_shipping']==0)*/, $au->user_rights->CheckAccess('w',329));
	
	$filetext=$ffg->ShowFiles('doc_file/list.html', $decorator,$from,$to_page,'inv_files.php', 'inv_file.html', 'swfupl-js/inv_files.php', $au->user_rights->CheckAccess('w',328), $au->user_rights->CheckAccess('w',330)/*&&($user['is_confirmed_shipping']==0)*/, $au->user_rights->CheckAccess('w',329), $folder_id,  $au->user_rights->CheckAccess('w',328),  $au->user_rights->CheckAccess('w',329),  $au->user_rights->CheckAccess('w',30),  $au->user_rights->CheckAccess('w',330),
	
	'',
	 $au->user_rights->CheckAccess('w',818),
	 $result
	...
	$sm->assign('bill',$user);
	$content=$sm->fetch('inv/files.html');
