...
$_user=new AccInItem;
$user=$_user->GetItemById($fmi['acceptance_id']);

if($fmi['folder_id']==0){
	$_folder_desrc='';
}else{
	$_ff=new FileDocFolderItem(3, $fmi['acceptance_id'], new AccInFileItem(3));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл поступления',NULL,659, NULL,SecStr('файл '.$fmi['orig_name'].' для поступления № '.  $user['id'].$_folder_desrc),$user['id']);
