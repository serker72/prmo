...
require_once('classes/pl_posfileitem.php');
require_once('classes/pl_posfilegroup.php');
...
$smarty->assign("SITETITLE",'Файлы прайс-листа');
...
if(!$au->user_rights->CheckAccess('w',600)){ //192)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	...
	$_file=new PlPosFileItem(4);
	$file=$_file->GetItemById($id);
	
	if($file!==false){
		$_file->Del($id);
		
		$log->PutEntry($result['id'],'удалил файл',NULL,603,NULL,'имя файла '.SecStr($file['orig_name']),$bill_id);
	}
	
	header("Location: pl_files.php?bill_id=".$bill_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	$_ff=new FileDocFolderItem(4, $bill_id, new PlPosFileItem(4));
	$_ff->SetTablename('pl_position_file_folder');
	...
	if($file!==false){
		$_ff->Del($id,$file,$result,603);
		
		$log->PutEntry($result['id'],'удалил папку',NULL,603,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
	
	header("Location:pl_files.php?bill_id=".$bill_id.'&folder_id='.$folder_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==4)){
	...
	if(is_array($files)&&(count($files)>0)){
		foreach($files as $v){	
		    ...
		    $_ff->Edit($v, array('parent_id'=>$move_folder_id), NULL,$result, 603);
...
unset($smarty);
	...
	*/
	...
	$ffg=new PlPosFileGroup(4,  $bill_id,  new FileDocFolderItem(4,  $bill_id, new PlPosFileItem(4)));
	
	//($template, DBDecorator $dec,$from=0,$to_page=ITEMS_PER_PAGE,$pagename='files.php', $loadname='load.html', $uploader_name='/swfupl-js/files.php', $can_load=false, $can_delete=false, $can_edit=false, $folder_id=0, $can_create_folder=false, $can_edit_folder=false, $can_delete_folder=false, $can_move_folder=false, $id_prefix=''){
	
	
	$filetext=$ffg->ShowFiles('doc_file/list.html', $decorator,$from,$to_page,'pl_files.php', 'pl_file.html', 'swfupl-js/pl_files.php',   
	$au->user_rights->CheckAccess('w',601),
	 $au->user_rights->CheckAccess('w',603),
	  $au->user_rights->CheckAccess('w',602),  $folder_id,  
	  $au->user_rights->CheckAccess('w',601),  $au->user_rights->CheckAccess('w',602),  $au->user_rights->CheckAccess('w',603),  $au->user_rights->CheckAccess('w',603) );
