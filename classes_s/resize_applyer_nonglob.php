	...
	public function MakePhoto($file,$action,$folder,$addtoname=''){
		...
		if(@isset($action['doit'])||(@$action['doit'])){
			...
			if($action['do_resize']){
				...
				$newname=$folder.'/'.$pre.$r->MakeNewName($addtoname, iconv("utf-8","windows-1251",$file['name']));
				//echo $newname;
				...
				//	$pref = time();
					$newname=SecureCyr(eregi_replace('([[:alnum:]])(\\.[[:alnum:].]*)?$','\\1'.'-'.$addtoname.$extension,iconv("utf-8","windows-1251",$file['name'])));
