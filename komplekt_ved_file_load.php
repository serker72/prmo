...
$_user=new PosItem;
$user=$_user->GetItemById($fmi['komplekt_ved_id']);

if($fmi['folder_id']==0){
	$_folder_desrc='';
}else{
	$_ff=new FileDocFolderItem(4, $fmi['komplekt_ved_id'], new KvFileItem(4));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл заявки',NULL,82, NULL,SecStr('файл '.$fmi['orig_name'].' для заявки '.  $user['id'].$_folder_desrc),$user['id']);
