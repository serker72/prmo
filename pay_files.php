...
$smarty->assign("SITETITLE",'Файлы исходящей оплаты');
...
$log->PutEntry($result['id'],'открыл реестр файлов исходящей оплаты',NULL,266, NULL, $user['code'].$_folder_desrc,$pay_id);	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил файл исходящей оплаты',NULL,613,NULL,'имя файла '.SecStr($file['orig_name']),$bill_id);
		
		$log->PutEntry($result['id'],'удалил файл',NULL,276,NULL,'имя файла '.SecStr($file['orig_name']),$itm['id']);
	}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил папку',NULL,276,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
