...
$smarty->assign("SITETITLE",'Файлы реализации');
...
if($folder_id==0){
	...
	$_ff=new FileDocFolderItem(4, $acc_id, new AccFileItem(4));
...
$log->PutEntry($result['id'],'открыл реестр файлов реализации',NULL,200, NULL, 'реализация № '.$user['id'].$_folder_desrc,$acc_id);	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	if(!$au->user_rights->CheckAccess('w',239)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил файл реализации',NULL,93,NULL,'имя файла '.SecStr($file['orig_name']),$bill_id);
		
		$log->PutEntry($result['id'],'удалил файл',NULL,239,NULL,'имя файла '.SecStr($file['orig_name']),$itm['id']);
	}
	
	header("Location: acc_files.php?acc_id=".$acc_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		$_ff->Del($id,$file,$result,239);
		
		$log->PutEntry($result['id'],'удалил папку',NULL,239,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
	
	header("Location:acc_files.php?acc_id=".$acc_id.'&folder_id='.$folder_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==4)){
	...
	if(is_array($files)&&(count($files)>0)){
		foreach($files as $v){	
			...
			$_ff=new FileDocFolderItem(4, $ship_id, new AccFileItem(4));
	$_ff->SetTablename('acceptance_file_folder');
	$_ff->SetDocIdName('acc_id');
		
		    $_ff->Edit($v, array('parent_id'=>$move_folder_id), NULL,$result, 239);
...
$_menu_id=34;
	...
	$ffg=new AccFileGroup(4, $acc_id, new FileDocFolderItem(4,  $bill_id, new AccFileItem(4)));
	
	$filetext=$ffg->ShowFiles('doc_file/list.html', $decorator,$from,$to_page,'acc_files.php', 'acc_file.html', 'swfupl-js/acc_files.php', $au->user_rights->CheckAccess('w',237), $au->user_rights->CheckAccess('w',239), $au->user_rights->CheckAccess('w',238), $folder_id, $au->user_rights->CheckAccess('w',237), $au->user_rights->CheckAccess('w',238), $au->user_rights->CheckAccess('w',239), $au->user_rights->CheckAccess('w',239),
	...
	/*$ffg=new AccFileGroup(4);
	
	$filetext=$ffg->ShowFiles($acc_id,'acc/files_list.html', $decorator,$from,$to_page,'acc_files.php', 'acc_file.html', 'swfupl-js/acc_files.php', $au->user_rights->CheckAccess('w',237), $au->user_rights->CheckAccess('w',239), $au->user_rights->CheckAccess('w',238));*/
	...
	$sm->assign('acc',$user);
	$content=$sm->fetch('acc/files.html');
