...
$user=$_user->GetItemById($fmi['payment_id']);

if($fmi['folder_id']==0){
	$_folder_desrc='';
}else{
	$_ff=new FileDocFolderItem(1, $fmi['payment_id'], new CashInItem(1));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл прихода наличных',NULL,883, NULL,SecStr('файл '.$fmi['orig_name'].' для '.  $user['code'].$_folder_desrc),$user['id']); 
