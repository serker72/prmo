<?php
	...
	if(isset($_FILES['Filedata'])){
		...
		$code=$_fi->Add(array('invcalc_id'=>$bill_id,'user_id'=>$result['id'], 'orig_name'=>SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])), 'folder_id'=>abs((int)$_POST['folder_id']), 'filename'=>basename($tempname), 'pdate'=>time()));
		
		$log->PutEntry( $result['id'],'добавил файл распоряжения на инвентаризацию',NULL,455,NULL,'имя файла '.SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])),$bill_id);
