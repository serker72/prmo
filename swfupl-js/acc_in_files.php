<?php
	...
	if(!$au->user_rights->CheckAccess('w',668)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	...
	$_fi=new AccInFileItem;
	
	$_ai=new AccInItem;
	$_bill=new BillInItem;
	
	$ai=$_ai->GetItemById($acc_id);
	...
	if(isset($_FILES['Filedata'])){
		...
		$code=$_fi->Add(array('acceptance_id'=>$acc_id,'user_id'=>$result['id'], 'orig_name'=>SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])), 'folder_id'=>abs((int)$_POST['folder_id']), 'filename'=>basename($tempname), 'pdate'=>time()));
		
		$log->PutEntry( $result['id'],'добавил файл реализации',NULL,613,NULL,'имя файла '.SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])),$ai['bill_id']);
		
		$log->PutEntry( $result['id'],'добавил файл реализации',NULL,644,NULL,'имя файла '.SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])),$ai['sh_i_id']);
		
		$log->PutEntry( $result['id'],'добавил файл реализации',NULL,668,NULL,'имя файла '.SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])),$ai['id']);
