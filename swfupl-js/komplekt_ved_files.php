<?php
	...
	$ai=$_ai->GetItemById($komplekt_ved_id);
	...
	if(isset($_FILES['Filedata'])){
		...
		$code=$_fi->Add(array('komplekt_ved_id'=>$komplekt_ved_id,'user_id'=>$result['id'], 'orig_name'=>SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])), 'folder_id'=>abs((int)$_POST['folder_id']), 'filename'=>basename($tempname), 'pdate'=>time()));
		
		$log->PutEntry( $result['id'],'добавил файл заявки',NULL,298,NULL,'имя файла '.SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])),$ai['id']);
