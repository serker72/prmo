<?php
	...
	if(isset($_FILES['Filedata'])){
		...
		$code=$_fi->Add(array('user_id'=>$result['id'], 'org_id'=>$result['org_id'], 'orig_name'=>SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])), 'folder_id'=>abs((int)$_POST['folder_id']),  'filename'=>basename($tempname), 'pdate'=>time()));
		
		$log->PutEntry($result['id'],'добавил файл в раздел Файлы и документы - Справочная информация',NULL,31,NULL,'имя файла '.SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])),$code);
