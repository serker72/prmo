<?php
	...
	//	move_uploaded_file ( $_FILES['Filedata']['tmp_name'], $tempname);
		...
		$code=$_fi->Add(array('bill_id'=>$id,'user_id'=>$result['id'], 'orig_name'=>SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])), 'folder_id'=>abs((int)$_POST['folder_id']), 'filename'=>basename($tempname), 'pdate'=>time()));
		
		$log->PutEntry( $result['id'],'добавил файл планировщика',NULL,905,NULL,'имя файла '.SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])), $id);
