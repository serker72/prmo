...
require_once('classes/contractitem.php');
...
$smarty->assign("SITETITLE",'ƒоговор контрагента');
...
if($folder_id==0){
	...
	$_ff=new FileDocFolderItem(3, $sup_id, new ContractItem(3));
...
$log->PutEntry($result['id'],'открыл реестр договоров контрагента',NULL,91, NULL, $user['code'].' '.$user['full_name'].$_folder_desrc,$user_id);

//удаление файлов
if(isset($_GET['action'])&&($_GET['action']==2)){
	if(!$au->user_rights->CheckAccess('w',173)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	...
	$_file=new ContractItem;
	$file=$_file->GetItemById($id);
	
	if($file!==false){
		$_file->Del($id);
		
		$log->PutEntry($result['id'],'удалил договор',NULL,173,NULL,'им€ файла '.SecStr($file['orig_name']),$sup_id);
	}
	
	header("Location: contracts.php?sup_id=".$sup_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		$_ff->Del($id,$file,$result,173);
		
		$log->PutEntry($result['id'],'удалил папку',NULL,170,NULL,'им€ папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
	
	header("Location:contracts.php?sup_id=".$sup_id.'&folder_id='.$folder_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==4)){
	...
	$_ff->SetDocIdName('sup_id');
		    ...
		    $_ff->Edit($v, array('parent_id'=>$move_folder_id), NULL,$result, 173);
...
$_menu_id=31;
	...
	*/
	...
	$ffg=new ContractGroup(3, $sup_id,  new FileDocFolderItem(3,  $sup_id, new ContractItem(3)));
	
	$filetext=$ffg->ShowFiles('doc_file/list_email.html', $decorator,$from,$to_page,'contracts.php', 'contract.html', 'swfupl-js/contracts.php', $au->user_rights->CheckAccess('w',171), $au->user_rights->CheckAccess('w',173), $au->user_rights->CheckAccess('w',172), $folder_id,  $au->user_rights->CheckAccess('w',171),  $au->user_rights->CheckAccess('w',172),  $au->user_rights->CheckAccess('w',173),  $au->user_rights->CheckAccess('w',173) );
	...
	$sm->assign('user',$user);
	$content=$sm->fetch('suppliers/contracts.html');
